...
auto UIWindow::event(state::UIStatePtr& state) -> void
{
...
auto UIWindow::mouseMoveHook(const int32_t newX, const int32_t newY) -> void
{
    ...
    uiState_->hoveredId = state::NOTHING;
    ...
    if (uiState_->clickedId != state::NOTHING
...
auto UIWindow::mouseButtonHook(const uint32_t btn, const uint32_t action) -> void
{
    ...
    uiState_->hoveredZIndex = state::NOTHING;
    propagateHoverScanEvent();
    ...
    else if (btn == Input::LEFT && action == Input::RELEASE)
    {
        ...
        uiState_->clickedId = state::NOTHING;
...
auto UIWindow::mouseScrollHook(const uint32_t xOffset, const uint32_t yOffset) -> void
{
    ...
    propagateEventTo(MouseScrollEvt{}, uiState_->closestScroll);
...
auto UIWindow::propagateEventTo(const elementcomposable::IEvent& evt,
