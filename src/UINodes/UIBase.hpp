...
public:
    ...
    auto getBaseLayoutData() -> elementcomposable::LayoutBase&;
    auto getEventManager() -> elementcomposable::Events&;
...
protected:
    ...
    */
    virtual auto event(state::UIStatePtr& state) -> void = 0;
...
protected:
    ...
    elementcomposable::Events eventsMgr_;
...
template<typename T>
requires std::is_base_of_v<src::uinodes::UIBase, T>
struct std::formatter<std::shared_ptr<T>> : std::formatter<std::string_view>
{
    auto format(const src::uinodes::UIBasePtr& obj, format_context& ctx) const
    {
